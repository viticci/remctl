...
**Due dates and alarms.** `dueDate` comes from `ZDUEDATE`; `displayDate` from `ZDISPLAYDATEDATE` when present. Date-only input creates all-day reminders through date components. On `edit -d`, one absolute alarm equal to the old due time moves with it; `edit -d clear` removes such an alarm. Alarms are `ZREMCDOBJECT` rows (`Z_ENT = 15`) with triggers; relative, absolute, and location triggers serialize under `alarms`. Writing a location alarm replaces the previous one.

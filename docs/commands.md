...
- `dueDate` is the real due date. For an all-day reminder (`allDay: true`) it is midnight of that day, such as `2026-09-30T00:00:00`, in every time zone. `displayDate` appears when Reminders shows the reminder at another time than `dueDate`, for example because an alarm at a fixed date and time is still set to the old time. Reminders.app lists and labels the reminder at `displayDate`, so `today`, `overdue`, `upcoming`, and human output place it there too. `dueDate` stays the due date.
